...
ioTypes.multiboxClone = class extends IO {
    ...
    think(input) {
        if (this.body.multiboxMaster) {
            ...
            let angle = Math.PI / 12; // the angle to add to the orbit
...
ioTypes.nearestSameMaster = class extends IO {
    constructor(body) {
        super(body);
    ...
    validate(e, m, mm, sqrRange, sqrRangeMaster) {
        ...
        (this.body.master.master.team == e.master.master.team && e.master.master != this.body.master.master) &&
        (e.master.master.team !== -101 && e.master.master.team !== -100) &&
        ...
        (e.health.amount < e.health.max) &&
         (e.healer_dont_target == false || e.health.amount < e.health.max) &&
...
ioTypes.bossorbit = class extends IO {
    constructor(body) {
        super(body)
        this.turnwise = 1
    }
...
ioTypes.gotorm = class extends IO {
  constructor(body) {
    super(body)
    this.goal = (room.randomType('como'));
    this.timer = Math.random() * 500 | 0
    this.state = 1
  }
  think(input) {
    if (this.timer <= 0 || util.getDistance(this.body, this.goal) < this.body.SIZE || this.state === 0) {
        this.timer = Math.random() * 500 | 0;
        this.state = 1;
        this.goal = room.randomType(Math.random() > .9 ? "unco" : "rare");
      }
    return {
      goal: this.goal
    }
...
ioTypes.sandstorm = class extends IO {
    constructor(body) {
        super(body);
        this.timer = 0;
    }
